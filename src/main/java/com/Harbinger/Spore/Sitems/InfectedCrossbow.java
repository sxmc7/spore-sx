...
public class InfectedCrossbow extends CrossbowItem implements SporeWeaponData {
    ...
    private static boolean loadProjectile(LivingEntity entity, ItemStack stack, ItemStack itemStack, boolean p_40866_, boolean p_40867_) {
        if (itemStack.isEmpty()) {
            ...
            if (!flag && !p_40867_ && !p_40866_) {
                ...
                if (itemStack.isEmpty() && entity instanceof Player player) {
    ...
    private static void shootProjectile(Level level, LivingEntity entity, InteractionHand hand, ItemStack stack, ItemStack p_40899_, float p_40900_, boolean p_40901_, float p_40902_, float p_40903_, float p_40904_) {
        if (!level.isClientSide) {
            ...
            if (entity instanceof CrossbowAttackMob) {
                ...
                LivingEntity crossbowTarget = crossbowattackmob.getTarget();
                if (crossbowTarget == null) return;
                crossbowattackmob.shootCrossbowProjectile(crossbowTarget, stack, projectile, p_40904_);
            } else {
    ...
    public void appendHoverText(ItemStack stack, @Nullable Level p_40881_, List<Component> components, TooltipFlag p_40883_) {
        ...
        if (Screen.hasShiftDown()){
            ...
            components.add(Component.translatable("spore.item.desc."+desc).withStyle(ChatFormatting.RED));
        }else {
    ...
    public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
        // 清除真实无敌帧 — Spore 武器无视无敌帧
        com.Harbinger.Spore.coremod.CoreModHooks.clearRealInvuln(target);

        if (tooHurt(stack)){
            hurtTool(stack,attacker,1);
        }
        doEntityHurtAfterEffects(stack,target,attacker);
        boolean flag = super.hurtEnemy(stack, target, attacker);

        // === 和Spore生物完全对等的三重伤害通道（跳过创造模式玩家） ===
        if (!target.level().isClientSide && !(target instanceof Infected || target instanceof UtilityEntity)
                && !(target instanceof net.minecraft.world.entity.player.Player p && p.isCreative())) {
            if (SConfig.SERVER.enable_force_set_health.get() && !hasSporeArmor(target)) {
                ...
                target.getPersistentData().putLong("spore_frost_antiheal_time", target.level().getGameTime() + 60);
                float chaseDmg = Math.max(1.0f, (float)reduction * 0.3f);
            ...
            if (SConfig.SERVER.enable_percentage_damage.get()) {
                ...
                if (pct > 0) {
                    float f = (float) attacker.getAttributeValue(net.minecraft.world.entity.ai.attributes.Attributes.ATTACK_DAMAGE);
                    float percDmg = target.getMaxHealth() * (float)(pct / 100.0);
                    float totalForceDmg = f + percDmg;
                    UnsafeHealthHelper.addHealth(target, -totalForceDmg);
                    float chaseDmg = Math.max(1.0f, totalForceDmg * 0.3f);
                    target.getPersistentData().putFloat("spore_hp_keeper", chaseDmg);
                    target.getPersistentData().putInt("spore_hp_keeper_ticks", 20);
                }
            }
    ...
    private static boolean hasSporeArmor(LivingEntity entity) {
        for (ItemStack armorSlot : entity.getArmorSlots()) {
            if (armorSlot.getItem() instanceof SporeArmorData) {
                return true;
            }
        }
        return false;
